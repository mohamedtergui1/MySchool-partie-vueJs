...
onMounted(async () => {
    ...
    await classroomDetails().fetchStudents(id);

});
</script>

<template>
    <div class="flex  py-10 justify-center lg:justify-start  flex-wrap gap-10 px-5 ">
        <div v-for=" (s, index) in students" :key="index"
            class="px-2 pt-4 h-80 mb-10  flex flex-col  w-60 items-center text-center shadow-lg border-2 rounded-lg ">
            <img class="inline-flex object-cover border-4 border-indigo-600 rounded-full shadow-[5px_5px_0_0_rgba(0,0,0,1)] shadow-indigo-600/100 bg-indigo-50 text-indigo-600   !h-48 !w-48"
                :src="s.image ? baseUrlfroPicinClassroom + s.image : ''" alt="">
